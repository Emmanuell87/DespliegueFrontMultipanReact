...
const ProductosPromo = ({imagen, descuento, nombre, descripcion, precio}) => {
    return (
		...
		<div className="w-full md:w-1/3 p-6 flex flex-col flex-grow flex-shrink ">
			<div className="flex-1 bg-white rounded-t rounded-b-none overflow-hidden shadow-lg">
				<Link to="/" className="flex flex-wrap no-underline hover:no-underline">
					...
					<p className="w-full text-gray-600 text-xs md:text-sm px-6">{descuento}</p>
					<div className="w-full font-bold text-xl text-gray-900 px-6">{nombre}</div>
